...
public class Message {
     ...
     */
    public static  Message createAnswerMessage(RTCSessionDescription sessionDescription, boolean master, String recipientClientId) {

        String description = sessionDescription.sdp;

        String answerPayload = "{\"type\":\"answer\",\"sdp\":\"" + description.replace("\r\n", "\\r\\n") + "\"}";

        String encodedString = Base64.getEncoder().encodeToString(answerPayload.getBytes());
     ...
     */
    public static Message createOfferMessage(RTCSessionDescription sessionDescription, String clientId) {

        String description = sessionDescription.sdp;

        String offerPayload = "{\"type\":\"offer\",\"sdp\":\"" + description.replace("\r\n", "\\r\\n") + "\"}";
        String encodedString = Base64.getEncoder().encodeToString(offerPayload.getBytes());
