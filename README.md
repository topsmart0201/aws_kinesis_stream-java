...
Import the Maven project to your IDE, it will find dependency packagdees from Maven and build.

### Examples

#### Launching App

Run `AppMain.java` in `./src/main/source` with JVM arguments set to
```
...
Demo app will start running and putting sample video frames in a loop into Kinesis Video Streams. You can change your stream settings in `AppMain.java` before you run the app.
...
```
$ cd /<YOUR_FOLDER_PATH_WHERE_SDK_IS_DOWNLOADED>/AmazonKinesisStream/
```
