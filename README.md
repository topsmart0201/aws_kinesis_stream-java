...
* You can find available pre-built KinesisVideoProducerJNI library in [src/main/resources/lib/](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-java/tree/master/src/main/resources/lib) for Mac (x64), Ubuntu (x64) and Raspian (x86) and Windows 10. If pre-built libraries did not work for you, ["KinesisVideoProducerJNI"](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-cpp) native library needs to be built first before running the Java application. Please follow the steps  in the section **Build the native library (KinesisVideoProducerJNI) to run Java App** in Producer SDK CPP [readme](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-cpp).
...
#### Launching app sample application

Run `AppMain.java` in `./src/main/` with JVM arguments set to
```
...
**Note**: NativeLibraryPath must contain your ["KinesisVideoProducerJNI"](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-cpp#build-the-native-library-kinesisvideoproducerjni-to-run-java-app) library. File name depends on your Operating System:
...
app will start running and putting sample video frames in a loop into Kinesis Video Streams. You can change your stream settings in `AppMain.java` before you run the app.

##### Run the application from command line

If you want to run the `AppMain`, follow the [steps](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-java/issues/14) below. See [Prerequisites](https://github.com/awslabs/amazon-kinesis-video-streams-producer-sdk-java#prerequisites) to find available native library needed to run `AppMain`.
...
Compile and assemble Java SDK, Java app and the Maven dependencies
```
...
Start the app
```
$ java -classpath target/amazon-kinesis-video-streams-producer-sdk-java-1.10.0-jar-with-dependencies.jar -Daws.accessKeyId=<ACCESS_KEY> -Daws.secretKey=<SECRET_KEY> -Dkvs-stream=<KINESIS_VIDEO_STREAM_NAME> -Djava.library.path=<NativeLibraryPath> com.amazonaws.kinesisvideo.app.AppMain

```

##### Run the application from Docker

Refer the **README.md** file in the  *dockerscripts* folder for running the build and app within Docker container.

#### Launching PutMedia sample application

 Run `PutMedia.java` to send sample mkv stream to Kinesis Video Streams. **Note:** ACCESS_KEY, SECRET_KEY, and a KINESIS_VIDEO_STREAM are required for running this sample application as well. However, this application does not require JNI.
...
##### [Kinesis Video Streams Android](https://github.com/awslabs/aws-sdk-android-samples/tree/master/AmazonKinesisVideoApp)
